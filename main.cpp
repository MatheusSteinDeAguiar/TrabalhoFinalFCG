// O arquivo bibliotecas.h contem todas as bibliotecas necessárias para fazer o trabalho
// todos os arquivos .cpp que fazem alguma implementação relevante utilizam esse arquivo
...
float cam_pos_x = 0.00f;
float cam_pos_y = 0.00f;
float cam_pos_z = 2.00f;

// Variaveis utilizadas para contar quanto tempo se passou desde o ultimo frame renderizado
float delta_t = 0.0f;
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 00324300 - Eduardo Birkheuer da Silva", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
...
void camera()
{
    ...
    else
    {
        ...
        // e ScrollCallback().
        float r = g_CameraDistance;
        float y = cam_pos_y - r*sin(g_CameraPhi);
        float z = cam_pos_z - r*cos(g_CameraPhi)*cos(g_CameraTheta);
        float x = cam_pos_x - r*cos(g_CameraPhi)*sin(g_CameraTheta);
        // A adição da posição da camera nos calculos das coordenadas esfericas fazem com que
        // o calculo do vetor view resulte apenas na pura coordenada esferica (veja abaixo)



        // Abaixo definimos as varáveis que efetivamente definem a câmera virtual.
        // Veja slides 195-227 e 229-234 do documento Aula_08_Sistemas_de_Coordenadas.pdf.
        glm::vec4 camera_position_c  = glm::vec4(cam_pos_x, cam_pos_y, cam_pos_z,1.0f); // Ponto "c", centro da câmera
        glm::vec4 camera_lookat_l    = glm::vec4(x,y,z,1.0f); // Ponto "l", para onde a câmera (look-at) estará sempre olhando, dado em coordenadas esfericas
        // o calculo da vetor view será o seguinte
        // view.x = cam_pos_x - r*cos(g_CameraPhi)*sin(g_CameraTheta) - cam_pos_x = -r*cos(g_CameraPhi)*sin(g_CameraTheta)
        // view.y = cam_pos_y - r*sin(g_CameraPhi) - cam_pos_y                    = -r*sin(g_CameraPhi)
        // view.z = cam_pos_z - r*cos(g_CameraPhi)*cos(g_CameraTheta) - cam_pos_z = -r*cos(g_CameraPhi)*cos(g_CameraTheta)
        // com isso, o vetor view estará definido apenas por coordenadas esfericas
        glm::vec4 camera_view_vector =  camera_lookat_l - camera_position_c; // Vetor "view", sentido para onde a câmera está virada
        glm::vec4 camera_up_vector   = glm::vec4(0.0f,1.0f,0.0f,0.0f); // Vetor "up" fixado para apontar para o "céu" (eito Y global)

        // definição dos vetores W e U que serão utilizados para atualizar a posição da camera quando o usuário controlar o movimento WASD
        glm::vec4 vetor_w  = -camera_view_vector;
        glm::vec4 vetor_u  = glm::normalize(crossproduct(camera_view_vector, camera_up_vector));


        // calculo do tempo decorrido desde o ultimo frame
        float currentFrame = (float)glfwGetTime();
        delta_t = currentFrame - last_frame;
        last_frame = currentFrame;

        // definição da velocidade de movimento da camera com base no tempo decorrido desde o ultimo frame
        float velocidade_da_camera = 1.50f * delta_t;


        // alteração da posição da camera como é mostrado no slide 227 Aula 08 - Sistemas de Coordenadas
        if(W_press)
        {
            camera_position_c +=  -vetor_w * velocidade_da_camera;
        }
        if(S_press)
        {
            camera_position_c += vetor_w * velocidade_da_camera;
        ...
        if(A_press)
        {
           camera_position_c += -vetor_u * velocidade_da_camera;
        }
        if(D_press)
        {
            camera_position_c += vetor_u * velocidade_da_camera;
        ...
        cam_pos_x = camera_position_c.x;
        cam_pos_y = camera_position_c.y;
        cam_pos_z = camera_position_c.z;
